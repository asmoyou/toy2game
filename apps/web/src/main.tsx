...
import { LIBRARY_KEY, readLibrary, writeLibrary } from '@toy2game/catalog/browser';
import './style.css';
...
function App() {
  ...
  const [view, setView] = useState<View>(['recent', 'favorites'].includes(params.get('view') ?? '') ? params.get('view') as View : 'all');
  const [category, setCategory] = useState(categories.some(item => item.id === params.get('category')) ? params.get('category')! : 'all');
  const [query, setQuery] = useState(params.get('q') ?? '');
  const [sort, setSort] = useState<Sort>(['newest', 'title'].includes(params.get('sort') ?? '') ? params.get('sort') as Sort : 'recommended');
  ...
  useEffect(() => {
    ...
    document.title = `${viewNames[view]} · Toy2Game 在线玩具箱`;
