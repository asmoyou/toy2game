...
- 全部关卡按难度、最少步数递增排列，统一编号为第 1–120 关。初来乍到为第 1–30 关（4–6 步），渐入佳境为第 31–60 关（7–10 步），环环相扣为第 61–90 关（11–16 步），出库高手为第 91–120 关（17 步起）；每档各 30 关。
...
- `parking-escape-save-v2` 保存经过合法动作链校验的关卡进度、撤销历史与最佳成绩；本次统一编号后不迁移旧版进度和成绩，从第 1 关重新开始。`parking-escape-preferences-v1` 保存音效，继续沿用。损坏或无法访问的存储不妨碍开局。
...
关卡可通过 `npm run levels --workspace games/parking-escape` 从固定种子重新生成：种子 `20260907` 与 `20260908` 分别生成 24 与 96 个布局，合并后统一按难度、最少步数排序并连续编号。第二批按最少步数分配数量，并忽略车辆颜色与数组顺序去重；每个候选限制在 35,000 个搜索状态内，全部生成成功后才写入文件。生成需数分钟，运行后需重跑规则测试；测试会检查全部最短解、难度分布、每档连续编号及整套关卡的步数递增顺序。
...
详细浏览器检查覆盖 Chrome 与 WebKit 的 1440 × 1000、1180 × 820、820 × 1180、390 × 844 和 844 × 390 视口，包含画布像素、移车前后变化、四档难度下全部 120 关连续编号且可达、选关草稿取消与应用、选中关卡自动滚入视野、完整触控目标、撤销重做、暂停、刷新、通关与下一关。手机横屏的选关弹窗采用左右布局，滚动关卡时保留确认操作。`tests/edge-check.mjs` 补充第 30→31、60→61、90→91 关的跨难度接续、最难关卡的完整通关、减少动态效果、旧版进度重置、损坏或禁用存储、WebGL 不可用的恢复界面。截图输出到根目录 `artifacts/parking-escape/`。
