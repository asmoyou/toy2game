...
try {
  for (const level of [levels[23], levels.at(-1)]) {
    ...
    try {
      ...
      if (level.id === 24) {
        ...
        await expect(page.locator('#level-number')).toHaveText('第 25 关');
        await expect(page.locator('#move-count')).toHaveText('00');
        await page.reload();
        await expect(page.locator('#level-number')).toHaveText('第 25 关');
        await page.getByRole('button', { name: '游戏设置' }).click();
        await page.getByRole('button', { name: '出库高手', exact: true }).click();
        await expect(page.locator('[data-draft="24"]')).toHaveClass('completed');
        assert.deepEqual(errors, []);
        console.log('PASS original final level continues into the expansion, retains completion and survives reload');
        continue;
      }
