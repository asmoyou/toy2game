...
describe('verified parking challenges', () => {
  it('has 24 distinct, legal challenges using only the fifteen toy vehicles', () => {
    expect(LEVELS).toHaveLength(24);
    expect(new Set(LEVELS.map(level => JSON.stringify([level.cars, level.positions]))).size).toBe(24);
...
describe('legal moves and single-player engine', () => {
  ...
  it('finishes only after the police car has completely left and refuses later actions', () => {
    const solution = solve(level.cars, level.positions);
...
describe('validated progress persistence', () => {
  it('restores both a current path and its redo chain without changing the next move', () => {
    const level = LEVELS[4], client = createGame(level), solution = solve(level.cars, level.positions);
    if (solution.status !== 'solved') throw new Error('Missing solution');
    for (const move of solution.moves.slice(0, 3)) client.moves.slide(move);
    client.moves.back();
    const saved = { version: 1, level: level.id, game: client.getState()!.G, seconds: 37, best: { '1': 4 } };
