...
async function snapshot(page) {
  return page.evaluate(() => {
    const debug = window.__flip;
    return {
      game: JSON.parse(localStorage.getItem('flip-match-save-v1')),
...
async function prepare(page) {
  ...
  await page.getByRole('button', { name: '收起棋子列表', exact: true }).click();
  await page.getByRole('button', { name: '向右旋转视角' }).click();
  if (await page.evaluate(() => '__flip' in window)) await page.evaluate(() => { window.__flip.scene.camera.zoom = 1.1; window.__flip.scene.camera.updateProjectionMatrix(); });
}
...
async function checkToggle(page, externalExit) {
  ...
  await expect(page.locator('#fullscreen-button svg')).toHaveAttribute('data-lucide', 'maximize');
  ...
  try {
    ...
    for (const [name, width, height] of [['desktop', 1440, 1000], ['ipad-landscape', 1180, 820], ['ipad-portrait', 820, 1180], ['phone', 390, 844], ['phone-landscape', 844, 390]]) {
      await check(`${name} toolbar, settings and scene`, async page => {
        ...
        const png = PNG.sync.read(await page.locator('#match-scene canvas').screenshot()), colors = new Set();
