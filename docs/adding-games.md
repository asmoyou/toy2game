...
每款新游戏都必须在常用工具栏接入浏览器全屏切换，提供中文名称和 tooltip。验收须覆盖支持时进入与退出、外部退出后的按钮同步、请求失败反馈，以及不支持或权限禁用时隐藏；切换不能重置对局或相机。兼容旧版 Safari 的实现还需检查能力标志及成对接口，并验证手机和 iPad 横竖屏的工具栏尺寸与溢出。
